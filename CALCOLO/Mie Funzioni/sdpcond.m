...
[L,U,P] = lu(A);	% O(2/3 nˆ3)
while err > tol && iter < nmax
	iter = iter + 1;
	x = A * y;				% 2nˆ2−n ops
	y = x / norm(x);		% 2n+1 ops
	lambda = y' * A * y;	% 2nˆ2+n−1 ops
	z = fwsub(L,P*y_mu);	% nˆ2 ops
	x = bksub(U,z);			% nˆ2 ops
	y_mu = x / norm(x);		% 2n+1 ops
	mu = y_mu' * A * y_mu;	% 2nˆ2+n−1 ops
	K_new = lambda / mu;
	err = abs(K - K_new);
	K = K_new;
end

fprintf("\nStima numero di condizionamento spettrale (tol = %d, nnmax = %d)", tol, nmax);
K
end